 ...
 *
 * FDMI FOL record may be persisted on FDMI plugin side. In this case FDMI
 ...
 * be duplicates (i.e. FDMI FOL record about the same KV operation), and they
 ...
 *
 * The persistence couldn't grow indefinitely, so it should be a way to prune
 ...
 * rh_lsn_discarded for it's BE domain could be discarded from deduplication
...
static bool fol_fdmi_filter_match(struct m0_buf  *value,
...
M0_INTERNAL int
m0_fol_fdmi_filter_kv_substring(struct m0_conf_fdmi_filter   *filter,
                                ...
                                struct m0_fdmi_eval_var_info *var_info)
{
	...
	m0_tl_for(m0_rec_frag, &fol_rec->fr_frags, fol_frag) {
		...
		for (i = 0; i < cas_op->cg_rec.cr_nr; ++i) {
			...
			if (fol_fdmi_filter_match(&cas_rec->cr_val.u.ab_buf,
