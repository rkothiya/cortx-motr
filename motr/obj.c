...
m0__oo_instance(struct m0_op_obj *oo)
{
	...
	return m0__entity_instance(oo->oo_oc.oc_op.op_entity);
}
...
static bool obj_op_obj_invariant(struct m0_op_obj *oo)
{
	...
	/* Don't use a m0__xxx_instance, as they assert */
	if(oo == NULL)
		rc = false;
...
static bool obj_layout_id_invariant(uint64_t layout_id)
{
	...
	int      ltype = M0_OBJ_LAYOUT_TYPE(layout_id);
	uint64_t lid = M0_OBJ_LAYOUT_ID(layout_id);

	return M0_RC(M0_IN(ltype, (M0_LT_PDCLUST,
				   M0_LT_COMPOSITE,
				   M0_LT_CAPTURE)) &&
...
M0_INTERNAL uint64_t m0__obj_lid(struct m0_obj *obj)
{
	...
	M0_ENTRY();
	M0_PRE(obj != NULL);
	M0_PRE(obj_layout_id_invariant(obj->ob_attr.oa_layout_id));
	lid = M0_OBJ_LAYOUT_ID(obj->ob_attr.oa_layout_id);
	M0_LEAVE();
	return lid;
}
				  ...
				  uint64_t layout_id)
{
	M0_ENTRY();
	M0_PRE(obj != NULL);
	M0_PRE(obj_layout_id_invariant(layout_id));
...
static void obj_namei_cb_cancel(struct m0_op_common *oc)
{
	...
	M0_PRE(M0_IN(op->op_code, (M0_EO_CREATE,
				   M0_EO_DELETE,
				   M0_EO_OPEN)));
	...
	rc = m0__obj_namei_cancel(op);
	op->op_rc = rc;
	M0_LEAVE();
}

/**
 * 'launch entry' on the vtable for obj namespace manipulation. This client
...
static void obj_namei_cb_launch(struct m0_op_common *oc)
{
	...
	M0_PRE(M0_IN(op->op_code, (M0_EO_CREATE,
				   M0_EO_DELETE,
				   M0_EO_OPEN)));
	M0_PRE(M0_IN(op->op_entity->en_sm.sm_state, (M0_ES_INIT,
						     M0_ES_OPEN)));
	M0_PRE(m0_sm_group_is_locked(&op->op_sm_group));
	M0_ASSERT(entity_invariant_full(op->op_entity));
...
/**
 * 'op fini entry' on the vtable for entities. This client callback gets invoked
			 ...
			 struct m0_pool_version **pv)
{
	...
	if (m0__obj_pool_version_is_valid(obj)) {
...
m0__obj_layout_id_get(struct m0_op_obj *oo)
{
	...
	M0_ASSERT(obj_layout_id_invariant(obj->ob_attr.oa_layout_id));
	lid = M0_OBJ_LAYOUT_ID(obj->ob_attr.oa_layout_id);
 ...
 */
int obj_fid_make_name(char *name, size_t name_len,
		      const struct m0_fid *fid)
{
	int rc;
...
/**
 * Initialises a m0_obj namespace operation. The operation is intended
			     ...
			     struct m0_op *op)
{
	...
	M0_PRE(m0_op_common_invariant(oc));
	oo = bob_of(oc, struct m0_op_obj, oo_oc, &oo_bobtype);
	M0_PRE(m0_op_obj_invariant(oo));
	...
	oc->oc_cb_launch = obj_namei_cb_launch;
	oc->oc_cb_cancel = obj_namei_cb_cancel;
	oc->oc_cb_fini = obj_namei_cb_fini;
	oc->oc_cb_free = obj_namei_cb_free;

	/*
	 * Set the object's fid.
	 *
	 * Something about the fid at current motr implementation (fid_convert.h)
	 ...
	 * possible in current Motr implementation.
	 *
	 ...
	 *
	 * (3) Does Client need to check if an object's container id matches
...
static int obj_op_obj_init(struct m0_op_obj *oo)
{
	...
	M0_ENTRY();
	M0_PRE(oo != NULL);
	M0_PRE(M0_IN(OP_OBJ2CODE(oo), (M0_EO_CREATE,
				       M0_EO_DELETE,
				       M0_EO_OPEN)));
	...
	M0_POST(m0_op_obj_invariant(oo));
	return M0_RC(0);
}

/**
 * Prepares a client operation to be executed on an object. Does all the
			  ...
			  enum m0_entity_opcode opcode)
{
	...
	M0_PRE(M0_IN(entity->en_sm.sm_state, (M0_ES_INIT,
					      M0_ES_OPEN)));
	M0_PRE(op != NULL);
			   ...
			   enum m0_entity_opcode opcode)
{
	...
	M0_PRE(M0_IN(entity->en_sm.sm_state, (M0_ES_INIT,
					      M0_ES_OPEN)));
	M0_PRE(op != NULL);
	M0_PRE(M0_IN(opcode, (M0_EO_CREATE,
			      M0_EO_DELETE,
			      M0_EO_OPEN)));
	...
	case M0_ET_OBJ:
		...
		rc = obj_op_prepare(entity, op, opcode);
		if (rc != 0)
			goto error;

		/* Initialise the stuff specific to a obj namespace operation. */
		rc = obj_namei_op_init(entity, *op);
		if (rc != 0)
			goto op_fini;
		break;
	case M0_ET_IDX:
		     ...
		     struct m0_op **op)
{
	...
	if (entity->en_type == M0_ET_OBJ && pool != NULL) {
...
M0_EXPORTED(m0_obj_unit_size_to_layout_id);
...
M0_EXPORTED(m0_obj_layout_id_to_unit_size);
...
uint64_t m0_client_layout_id(const struct m0_client *instance)
{
	 ...
	 * solution.
	 */
	if (instance->m0c_config->mc_layout_id != 0)
		lid = instance->m0c_config->mc_layout_id;

	M0_LEAVE("lid=%"PRIu64, lid);
	return lid;
}
M0_EXPORTED(m0_client_layout_id);

enum m0_client_layout_type m0_obj_layout_type(struct m0_obj *obj)
{
	return M0_OBJ_LAYOUT_TYPE(obj->ob_attr.oa_layout_id);
}
M0_EXPORTED(m0_obj_layout_type);
		   ...
		   struct m0_op **op)
{
	...
	if (entity->en_type == M0_ET_IDX) {
		...
		m0_sm_move(&entity->en_sm, 0, M0_ES_OPENING);
		m0_sm_move(&entity->en_sm, 0, M0_ES_OPEN);
		m0_sm_group_unlock(&entity->en_sm_group);
		return M0_RC(0);
	} else
		return M0_RC(entity_namei_op(entity, op,
					     M0_EO_OPEN));
}
M0_EXPORTED(m0_entity_open);
